...
class data_run:
    ...
    def __init__(self, channel_trig, channel_spcm):
        self.channel_trig = channel_trig
        self.channel_spcm = channel_spcm

    def take_data(self, runtime):
        ...
        channels_to_enable = (1 << (self.channel_trig-1)) + (1 << (self.channel_spcm-1))
        ...
        while time.time() - start < runtime:
            ...
            self.timestamps = np.append(timestamps,timestamps_dummy[0:tags_valid.value])
            self.channels = np.append(channels,channels_dummy[0:tags_valid.value])
    ...
    def take_fake_data(self, runtime):
        ...
        for _ in range(0,num_loops):
            ...
            # Grab tags and stick them into the arrays we've made
            base.TDC_getLastTimestamps( 1, timestamps_dummy, channels_dummy, ctypes.byref(tags_valid))
            self.timestamps = np.append(self.timestamps,timestamps_dummy[0:tags_valid.value])
            self.channels = np.append(self.channels,channels_dummy[0:tags_valid.value])
    ...
    def process_histogram(self, max_time, bin_width):
        
        # Sort the tags so we know what came from what
        tags_trig = np.array(self.timestamps)[np.array(self.channels) == (self.channel_trig-1)]
        tags_spcm = np.array(self.timestamps)[np.array(self.channels) == (self.channel_spcm-1)]
        ...
        self.coinc = np.zeros(max_time_int+1)
        self.tau = np.array(range(len(self.coinc)))*act_bin_width_int*tagger_width
        ...
        for k in range(len(rescaled_trig)):
            ...
            if upper_pointer>=lower_pointer:
                for l in range(lower_pointer,upper_pointer + 1):
                    ...
                    self.coinc[coinc_index] += 1
    
    # Save data to file
    def save_data(self, filename, notes=None):
        # Save to HDF5
        h5f = h5py.File(filename, "w")
        h5f.create_group("Raw_Data")
        h5f.create_group("Information")
        # Saving raw data in case we wanna re-process
        h5f.create_dataset("Raw_Data/tags",data=self.timestamps,dtype='i8')
        h5f.create_dataset("Raw_Data/channels",data=self.channels,dtype='i1')
        h5f.create_dataset("Information/channel_trig",data=self.channel_trig,dtype='i1')
        h5f.create_dataset("Information/channel_spcm",data=self.channel_spcm,dtype='i1')
        ...
        h5f.create_dataset("Processed_Data/time",data=self.tau,dtype='f8')
        h5f.create_dataset("Processed_Data/coincidences",data=self.coinc,dtype='i8')
        h5f.close()
    
    # Load data from file
    def load_data(self, filename, read_notes=False):
        # Load from HDF5
        h5f = h5py.File(filename, "r")
        # Grab all the data
        self.timestamps = np.array(h5f["Raw_Data/tags"][()])
        self.channels = np.array(h5f["Raw_Data/channels"][()])
        self.channel_trig = np.array(h5f["Information/channel_trig"][()])
        self.channel_spcm = np.array(h5f["Information/channel_spcm"][()])
        self.tau = np.array(h5f["Processed_Data/time"][()])
        self.coinc = np.array(h5f["Processed_Data/coincidences"][()])
        if read_notes:
            try:
                notes = h5f['Information/Notes'][()]
                print(notes)
            except:
                print("File doesn't conatin notes")
        h5f.close()
...
# def save_data(filename, timestamps, channels, channel_trig=None, channel_spcm=None, notes=None, tau=None, coinc=None):
#     # Save to HDF5
#     h5f = h5py.File(filename, "w")
#     h5f.create_group("Raw_Data")
#     h5f.create_group("Information")
